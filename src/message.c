...
static struct selector parse_window_selector(FILE *rsp, char **message, struct window *acting_window, bool optional)
{
    ...
    if (value.type == TOKEN_TYPE_INT) {
        ...
        if (token_equals(result.token, ARGUMENT_COMMON_SEL_NORTH)) {
            ...
            if (acting_window) {
                struct window *closest_window = window_manager_find_managed_window_above(&g_window_manager, acting_window);
                if (closest_window) {
                    result.window = closest_window;
                } else {
            ...
            if (acting_window) {
                struct window *closest_window = window_manager_find_managed_window_below(&g_window_manager, acting_window);
                if (closest_window) {
                    result.window = closest_window;
                } else {
